...
abstract class Anticaptcha
{
    ...
    public function setLanguagePool($langPool)
    {
        switch ($langPool) {
            case'rn':
                $this->languagePool = $langPool;
                break;
            default:
    ...
    public function createTask()
    {
        ...
        if ($submitResult->errorId == 0) {
            ...
            $this->debout(
                "API error {$submitResult->errorCode} : {$submitResult->errorDescription}", 'red');
    ...
    public function waitForResult($maxSeconds = 60, $currentSecond = 0)
    {
        ...
        if ($this->taskInfo->errorId == 0) {
            ...
            if ($this->taskInfo->status == "ready") {
                ...
                $this->getTaskSolution();
                return true;
            }
            $this->debout('unknown API status, update your software', 'red');
            return false;
    ...
    protected function debout($message, $color = 'white')
    {
        if (!$this->verboseMode) {
            return false;
        }
    ...
    private function jsonPostRequest($methodName, $postData)
    {
        $ch = curl_init();
